  ...
  private validatePreferences(data: unknown): UserPreferences {
    ...
    if (data && typeof data === 'object') {
      ...
      if (typeof obj.notificationsEnabled === 'boolean') {
        base.notificationsEnabled = obj.notificationsEnabled;
      }
      
      if (typeof obj.fontSize === 'number' && 
          obj.fontSize >= MIN_FONT_SIZE && 
          obj.fontSize <= MAX_FONT_SIZE) {
        base.fontSize = Math.round(obj.fontSize);
      }
    }
...
class UserPreferencesManager {
  ...
  private notifyPreferencesChange(
    ...
    if (oldPrefs.theme !== newPrefs.theme) {
      changes.push(`theme changed to ${newPrefs.theme}`);
    }
    
    if (oldPrefs.language !== newPrefs.language) {
      changes.push(`language changed to ${newPrefs.language}`);
    }
    
    if (oldPrefs.fontSize !== newPrefs.fontSize) {
      changes.push(`font size changed to ${newPrefs.fontSize}`);
    }
  ...
  private validatePreferences(data: unknown): UserPreferences {
    ...
    if (data && typeof data === 'object') {
      ...
      if (obj.language && VALID_LANGUAGES.includes(obj.language as string)) {
        result.language = obj.language as string;
      }
