...
class UserPreferencesValidator {
  ...
  static validateBatch(preferencesArray: Partial<UserPreferences>[]): UserPreferences[] {
    ...
    for (const [index, pref] of preferencesArray.entries()) {
      try {
        results.push(this.validate(pref));
      } catch (error) {
        if (error instanceof PreferenceError) {
          errors.push(error);
          console.warn(`Validation failed for item ${index}: ${error.message}`);
        } else {
          throw error;
        }
      }
    ...
    if (errors.length > 0) {
      console.error(`Batch validation completed with ${errors.length} error(s)`);
    }
