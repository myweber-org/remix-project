...
class UserPreferencesManager {
  ...
  private validatePreferences(data: any): UserPreferences {
    ...
    const fontSize = typeof data.fontSize === 'number' && data.fontSize >= 8 && data.fontSize <= 32 ? data.fontSize : 14;
...
class UserPreferencesManager {
  ...
  private validatePreferences(prefs: Partial<UserPreferences>): Partial<UserPreferences> {
    ...
    if (prefs.theme !== undefined) {
      validated.theme = ['light', 'dark', 'auto'].includes(prefs.theme) 
        ? prefs.theme as UserPreferences['theme']
        : DEFAULT_PREFERENCES.theme;
    }

    if (prefs.notifications !== undefined) {
      validated.notifications = typeof prefs.notifications === 'boolean' 
        ? prefs.notifications 
        : DEFAULT_PREFERENCES.notifications;
    }

    if (prefs.language !== undefined) {
      validated.language = VALID_LANGUAGES.includes(prefs.language) 
        ? prefs.language 
    ...
    if (prefs.resultsPerPage !== undefined) {
      ...
      validated.resultsPerPage = !isNaN(num) && num >= MIN_RESULTS_PER_PAGE && num <= MAX_RESULTS_PER_PAGE
        ? num
