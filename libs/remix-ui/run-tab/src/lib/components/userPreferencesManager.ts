...
class UserPreferencesManager {
  ...
  updatePreferences(updates: Partial<UserPreferences>): void {
    ...
    const changed = Object.keys(updates).some(key => 
      previousPreferences[key as keyof UserPreferences] !== this.preferences[key as keyof UserPreferences]
    );
...
class UserPreferencesManager {
  ...
  private validatePreferences(data: unknown): UserPreferences {
    ...
    if (data && typeof data === 'object') {
      ...
      if (typeof obj.resultsPerPage === 'number' && obj.resultsPerPage >= 5 && obj.resultsPerPage <= 100) {
        result.resultsPerPage = Math.floor(obj.resultsPerPage);
      }
    }
