...
class UserPreferencesManager {
  ...
  private validateAndMerge(partialPrefs: Partial<UserPreferences>): UserPreferences {
    ...
    if (!/^[a-z]{2}-[A-Z]{2}$/.test(merged.language)) {
      merged.language = UserPreferencesManager.DEFAULT_PREFERENCES.language;
    }

    // Validate numeric ranges
    merged.fontSize = Math.max(8, Math.min(48, merged.fontSize));
    merged.autoSaveInterval = Math.max(1000, Math.min(300000, merged.autoSaveInterval));
...
console.log('Current preferences:', prefsManager.getPreferences());
