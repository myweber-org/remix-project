...
class UserPreferencesManager {
  ...
  private validateAndMerge(partial: Partial<UserPreferences>): UserPreferences {
    ...
    merged.fontSize = Math.max(
      MIN_FONT_SIZE,
      Math.min(MAX_FONT_SIZE, Number(merged.fontSize) || DEFAULT_PREFERENCES.fontSize)
    );

    merged.autoSaveInterval = Math.max(
      MIN_AUTO_SAVE_INTERVAL,
      Math.min(MAX_AUTO_SAVE_INTERVAL, Number(merged.autoSaveInterval) || DEFAULT_PREFERENCES.autoSaveInterval)
    );
